...
_splitter = "\\" if sys.platform == "win32" else "/"
...
def load(path):
    ...
    for n,file in enumerate(ls):
        ...
        pprint(n+1, len(ls), path.split(_splitter)[-1])
    return qt
...
def save(path, qt):
    ...
    for i in range(div[0]):
        with open(path+f"{_splitter}qt_{str(i)}.pickle", "wb") as f:
            pickle.dump(dict(itertools.islice(qt.items(), max_file_size*i, max_file_size+max_file_size*i)),f, pickle.HIGHEST_PROTOCOL)
        pprint(i+1, div[0]+1, path.split(_splitter)[-1])
    if div[1] > 0:
        with open(path+f"{_splitter}qt_{str(i+1)}.pickle", "wb") as f:
            pickle.dump(dict(itertools.islice(qt.items(), len(qt)-div[1], len(qt))),f, pickle.HIGHEST_PROTOCOL)
        pprint(i+(1 if div[0]==0 else 2), div[0]+1, path.split(_splitter)[-1])